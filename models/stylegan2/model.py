...
class Generator_(nn.Module):
    def __init__(
        ...
        self.channels = [4, 8, 8, 16, 16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 512, 1024, 1024]
        for i in range(self.num_layers):

            self.scales.append(
                nn.Sequential(
                    nn.Linear(512, 512), nn.LayerNorm(512), nn.LeakyReLU(), nn.Linear(512, self.channels[i])
                )
            )
            self.shifts.append(
                nn.Sequential(
    ...
    def forward(
        ...
        # else:
        #     for i in range(self.num_layers):
        #         scale = self.scales[i](w_text.float()).view(-1)
        #         shift = self.shifts[i](w_text.float()).view(-1)
        #         noise[i] = scale * noise[i] + shift
...
class Generator(nn.Module):
    def __init__(
        ...
        self.n_latent = self.log_size * 2 - 2
        # self.affineBlocks = nn.ModuleList()
        # self.channels = [512, 512, 512, 512, 512, 512, 512, 512, 512, 256, 256, 128, 128, 64, 64, 32, 32]
        # for i in range(self.num_layers):
        #     self.affineBlocks.append(AffineBlock(self.channels[i]))
...
class AffineBlock(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.affine1 = Affine(channels)
        self.affine2 = Affine(channels)
        self.lrelu1 = nn.LeakyReLU(0.2, inplace=True)
        self.lrelu2 = nn.LeakyReLU(0.2, inplace=True)

    def forward(self, x, w_text):
        # shape of x and w_text are (b,channels,h,w), (1,512), respectively
        x = self.lrelu1(self.affine1(x, w_text))
        x = self.lrelu2(self.affine2(x, w_text))
        return self.conv(x)
...
class Affine(nn.Module):
    ...
    def __init__(self, num_features):
        ...
        self.fc_gamma = nn.Sequential(OrderedDict([
            ('linear1',nn.Linear(512, num_features)),
