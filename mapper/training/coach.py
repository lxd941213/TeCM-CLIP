...
from criteria import id_loss, face_loss
...
class Coach:
	def __init__(self, opts):
		...
		if self.opts.clip_lambda > 0:
			self.clip_loss = clip_loss.CLIPLoss(opts, self.clip_model)
		if self.opts.latent_l2_lambda > 0:
			self.latent_l2_loss = nn.MSELoss().to(self.device).eval()
		if self.opts.face_l2_lambda > 0:
			self.face_l2_loss = face_loss.FaceLoss(self.opts).to(self.device).eval()
		if self.opts.bg_l2_lambda > 0:
			self.bg_l2_loss = face_loss.BackGroundLoss(self.opts).to(self.device).eval()
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w, style_text_embedding = batch
				# w = batch
				w = w.to(self.device)
				style_text_embedding = style_text_embedding.to(self.device)
				with torch.no_grad():
					x, rgbs = self.net.decoder([w], input_is_latent=True, randomize_noise=False, truncation=1,
											input_is_stylespace=self.opts.work_in_stylespace)
				# w_hat = w + 0.1 * self.net.mapper(w)
				# text_embedding = self.clip_model.encode_text(self.text_inputs).view(1, 1, -1)  # 1*512
				text_embedding = self.clip_model.encode_text(style_text_embedding).view(-1, 1, 512)  # 1*1*512
				w_plus_plus = self.net.mapper(w, text_embedding)
				w_plus_plus = w_plus_plus * 0.1 + w * 0.9
				x_hat, w_plus_plus, rgbs_hat = self.net.decoder([w_plus_plus], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				loss, loss_dict = self.calc_loss(w, w_plus_plus, rgbs, rgbs_hat, style_text_embedding)
				loss.backward()
				self.optimizer.step()
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				x, rgbs = self.net.decoder([w], input_is_latent=True, randomize_noise=False, truncation=1, input_is_stylespace=self.opts.work_in_stylespace)
				# w_hat = w + 0.1 * self.net.mapper(w)
				# text_embedding = self.clip_model.encode_text(self.text_inputs)  # 1*512
				text_embedding = self.clip_model.encode_text(style_text_embedding).view(-1, 1, 512)  # 1*512
				w_plus_plus = self.net.mapper(w, text_embedding)
				w_plus_plus = w_plus_plus * 0.1 + w * 0.9
				x_hat, w_plus_plus, rgbs_hat = self.net.decoder([w_plus_plus], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				# loss, cur_loss_dict = self.calc_loss(w, w_plus, w_plus_plus, x, x_hat)
				loss, cur_loss_dict = self.calc_loss(w, w_plus_plus, rgbs, rgbs_hat, style_text_embedding)
	...
	def calc_loss(self, w, w_plus_plus, rgbs, rgbs_hat, style_text_embedding):
		loss_dict = {}
		loss = 0.0
		if self.opts.id_lambda > 0:
			# loss_id = 0.0
			# for i in range(len(rgbs)):
			# 	loss_id_, _ = self.id_loss(rgbs[i], rgbs_hat[i])
			# 	loss_id += loss_id_
			loss_id, sim_improvement = self.id_loss(rgbs[-1], rgbs_hat[-1])
			loss_dict['loss_id'] = float(loss_id)
			loss_dict['id_improve'] = float(sim_improvement)
			loss = loss_id * self.opts.id_lambda
		if self.opts.clip_lambda > 0:
			loss_clip = self.clip_loss(rgbs_hat[-1], style_text_embedding).mean()
			loss_dict['loss_clip'] = float(loss_clip)
			loss += loss_clip * self.opts.clip_lambda
		if self.opts.latent_l2_lambda > 0:
			loss_l2_latent = self.latent_l2_loss(w, w_plus_plus)
			loss_dict['loss_l2_latent'] = float(loss_l2_latent)
			loss += loss_l2_latent * self.opts.latent_l2_lambda
		if self.opts.face_l2_lambda > 0:
			loss_face = self.face_l2_loss(rgbs[-1], rgbs_hat[-1])
			loss_dict['loss_face'] = float(loss_face)
			loss += loss_face * self.opts.face_l2_lambda
		if self.opts.bg_l2_lambda > 0:
			loss_bg = self.bg_l2_loss(rgbs[-1], rgbs_hat[-1])
			loss_dict['loss_bg'] = float(loss_bg)
			loss += loss_bg * self.opts.bg_l2_lambda
